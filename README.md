...
**카드 · 일정 · 기록 · 프로필은 브라우저 localStorage 에만 남는다.** 다만 **문답 답변과 진료 후
메모 원문은 AI 정리를 위해 백엔드로 전송된다** — 그리고 품질 점검을 위해 서버에 보관된다
(2027-01-15 삭제). 진입 화면의 동의가 그 범위다(`index.html` 의 `.consent`).
